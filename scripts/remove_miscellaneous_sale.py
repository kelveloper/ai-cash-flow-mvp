#!/usr/bin/env python3
"""
Script to remove all transactions with description "Miscellaneous Sale" 
...
def remove_from_supabase():
    """Remove transactions with 'Miscellaneous Sale' description from Supabase"""
    print("🗑️  Removing 'Miscellaneous Sale' transactions from Supabase...")
    
    supabase = create_supabase_client()
    
    # Count transactions to be removed
    checking_count = supabase.table('checking_transactions').select('id').ilike('description', '%Miscellaneous Sale%').execute()
    credit_count = supabase.table('credit_transactions').select('id').ilike('description', '%Miscellaneous Sale%').execute()
    ...
    if total_to_remove == 0:
        print("✅ No 'Miscellaneous Sale' transactions found in Supabase")
        return 0
    
    # Remove from checking_transactions
    if len(checking_count.data) > 0:
        result = supabase.table('checking_transactions').delete().ilike('description', '%Miscellaneous Sale%').execute()
        print(f"✅ Removed {len(checking_count.data)} checking transactions")
    
    # Remove from credit_transactions
    if len(credit_count.data) > 0:
        result = supabase.table('credit_transactions').delete().ilike('description', '%Miscellaneous Sale%').execute()
...
def remove_from_csv_files():
    """Remove transactions with 'Miscellaneous Sale' description from CSV files"""
    print("\n🗑️  Removing 'Miscellaneous Sale' transactions from CSV files...")
    ...
    for csv_file in csv_files:
        ...
        try:
            ...
            # Remove rows with 'Miscellaneous Sale' in description
            df_cleaned = df[~df['description'].str.contains('Miscellaneous Sale', case=False, na=False)]
            new_count = len(df_cleaned)
            removed_count = original_count - new_count
            
            if removed_count > 0:
                # Create backup
                backup_file = f"{csv_file}.sale_backup"
                ...
                print(f"✅ {csv_file}: No 'Miscellaneous Sale' transactions found")
...
def main():
    """Main function"""
    print("🧹 Removing 'Miscellaneous Sale' transactions")
    print("=" * 50)
